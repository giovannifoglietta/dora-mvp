...
def get_working_slots(db: Session, practitioner_id, target_date: date, slot_minutes: int = 55):
    """Return all valid start times for a given practitioner on a given date,
    filtered by working hours and existing bookings."""
    ...
    slots = []
    for w in windows:
        start_dt = datetime.combine(target_date, _to_time(w["start"]))
        end_dt = datetime.combine(target_date, _to_time(w["end"]))
        cursor = start_dt
    ...
    def conflicts(slot_start: datetime) -> bool:
        ...
        for b in existing:
            b_start = b.starts_at.replace(tzinfo=None)
